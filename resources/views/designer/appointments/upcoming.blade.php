            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold">
                        <i class="fas fa-calendar-alt me-2" style="color: #ffde9f;"></i>
                        {{ __('dashboard.upcoming_appointments') }}
                    </h1>
                    <p class="mt-1" style="color: #ffde9f;">{{ __('dashboard.manage_your_upcoming_schedule') }}</p>
                </div>
                <div class="flex items-center gap-4">
                    <a href="{{ route('designer.appointments.index', ['status' => 'pending']) }}"
                        ...
                        <span class="hidden sm:block">{{ __('dashboard.pending_appointments') }}</span>
                    </a>
                    <a href="{{ route('designer.appointments.index', ['status' => 'accepted']) }}"
                        class="bg-white bg-opacity-20 hover:bg-opacity-30 text-white px-4 py-2 rounded-lg transition-all duration-200 flex items-center gap-2">
                        <i class="fas fa-calendar-check"></i>
                        <span class="hidden sm:block">{{ __('dashboard.accepted_appointments') }}</span>
                    </a>
                    <a href="{{ route('designer.appointments.index') }}"
                        class="bg-white bg-opacity-20 hover:bg-opacity-30 text-white px-4 py-2 rounded-lg transition-all duration-200 flex items-center gap-2">
                        <i class="fas fa-list"></i>
                        <span class="hidden sm:block">{{ __('dashboard.all_appointments') }}</span>
                    </a>
                    <button onclick="refreshAppointments()"
                        class="bg-white bg-opacity-20 hover:bg-opacity-30 text-white px-4 py-2 rounded-lg transition-all duration-200 flex items-center gap-2">
                        <i class="fas fa-sync-alt"></i>
                        <span class="hidden sm:block">{{ __('dashboard.refresh') }}</span>
                    </button>
                    <div class="hidden md:block">
                        <i class="fas fa-calendar-plus text-4xl" style="color: #ffde9f;"></i>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                <div class="flex flex-wrap items-center gap-4">
                    ...
                    <select
                        class="border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm"
                        onchange="filterAppointments(this.value)" id="filter-select">
                        <option value="all" {{ request('filter') == 'all' ? 'selected' : '' }}>
                            {{ __('dashboard.all_upcoming') }}</option>
                        <option value="tomorrow" {{ request('filter') == 'tomorrow' ? 'selected' : '' }}>
                            {{ __('dashboard.tomorrow') }}</option>
                        <option value="this_week" {{ request('filter') == 'this_week' ? 'selected' : '' }}>
                            {{ __('dashboard.this_week') }}</option>
                        <option value="next_week" {{ request('filter') == 'next_week' ? 'selected' : '' }}>
                            {{ __('dashboard.next_week') }}</option>
                        <option value="this_month" {{ request('filter') == 'this_month' ? 'selected' : '' }}>
                            {{ __('dashboard.this_month') }}</option>
                    </select>
                    <div class="flex items-center gap-2">
                        <label for="status-filter"
                            class="text-sm font-medium text-gray-700">{{ __('dashboard.status') }}:</label>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors" id="appointment-{{ $appointment->id }}">
                                ...
                                <td>
                                    <div>
                                        ...
                                        @if ($appointment->duration_minutes)
                                            <div class="text-xs text-gray-400">{{ $appointment->duration_minutes }}
                                                {{ __('dashboard.minutes') }}</div>
                                        @endif
                                    </div>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="flex items-center space-x-2">
                                        ...
                                        @if ($appointment->order)
                                            <button
                                                onclick="openQuickAddModal({{ $appointment->id }}, {{ $appointment->order->id }})"
                            ...
                            <tr>
                                <td colspan="6" class="text-center py-8">
                                    <div class="flex flex-col items-center">
                                        <i class="fas fa-calendar-times text-4xl mb-4"
