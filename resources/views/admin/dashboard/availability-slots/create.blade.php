...
@section('content')
    ...
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold" style="color: var(--brand-brown);">
                    {{ trans('admin.create_availability_slot') }}</h1>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1.5rem;">
                        ...
                        <div>
                            ...
                            <option value="">{{ trans('admin.select_duration') }}
                                ...
                                <option value="15" {{ old('slot_duration_minutes', 15) == 15 ? 'selected' : '' }}>15
                                    {{ trans('admin.minutes') }}</option>
                                <option value="30" {{ old('slot_duration_minutes') == 30 ? 'selected' : '' }}>30
                                    {{ trans('admin.minutes') }}</option>
                                <option value="45" {{ old('slot_duration_minutes') == 45 ? 'selected' : '' }}>45
                                    {{ trans('admin.minutes') }}</option>
                                <option value="60" {{ old('slot_duration_minutes') == 60 ? 'selected' : '' }}>60
                                    {{ trans('admin.minutes') }}</option>
                                <option value="90" {{ old('slot_duration_minutes') == 90 ? 'selected' : '' }}>90
                                    {{ trans('admin.minutes') }}</option>
                                <option value="120" {{ old('slot_duration_minutes') == 120 ? 'selected' : '' }}>120
                                    {{ trans('admin.minutes') }}</option>
                            </select>
                        ...
                        <div>
                            ...
                            <input type="time" name="start_time" id="start_time" value="{{ old('start_time') }}"
                                required class="form-control" @error('start_time') style="border-color: #ef4444;" @enderror>
                            @error('start_time')
                                <p class="validation-message">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div style="grid-column: 1 / -1;">
                            ...
                            <textarea name="notes" id="notes" rows="3" class="form-control"
                                @error('notes') style="border-color: #ef4444;" @enderror placeholder="{{ trans('admin.enter_notes_optional') }}">{{ old('notes') }}</textarea>
                            @error('notes')
                                <p class="validation-message">{{ $message }}</p>
                            @enderror
                            <p class="validation-message">{{ trans('admin.notes_help') }}</p>
                        </div>
                    </div>
