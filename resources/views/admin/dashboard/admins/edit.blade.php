...
@section('pageTitle', __('admin.edit_admin_title'))
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ __('admin.edit_admin_title') }}</h1>
            <p class="text-gray-600 mt-1">{{ __('admin.edit_admin_title') }}: {{ $admin->name }}</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('admin.admins.update', $admin) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- Password -->
                <div>
                    <label for="password" class="block text-sm font-medium text-gray-700 mb-2">{{ __('admin.new_password_optional') }}</label>
                    <input type="password" 
                           id="password" 
                           name="password" 
                           ...
                           style="focus:ring-color: #2a1e1e;">
                    <p class="text-sm text-gray-500 mt-1">{{ __('admin.leave_empty_no_change') }}</p>
                    @error('password')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
