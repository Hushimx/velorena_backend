    ...
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-gray-600">{{ __('admin.assign_highlights_to_product') }}: <strong>{{ $product->name }}</strong></p>
            </div>
            <div class="flex space-x-3">
                <a href="{{ route('admin.products.index') }}" class="btn btn-secondary">
                    <i class="fas fa-arrow-left"></i>
                    <span>{{ __('admin.back_to_products') }}</span>
                </a>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            <div class="flex items-center space-x-4">
                @if ($product->image && file_exists(public_path($product->image)))
                    <img class="h-20 w-20 rounded-lg object-cover" src="{{ asset($product->image) }}" alt="{{ $product->name }}">
                @else
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            <form action="{{ route('admin.products.store-highlights', $product) }}" method="POST">
                @csrf
                ...
                <div class="space-y-4">
                                ...
                                <div class="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                                    <label class="flex items-start space-x-3 cursor-pointer">
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <!-- Sort Order Input -->
                                            <div class="mt-2">
                                                <label class="block text-xs text-gray-600 mb-1">{{ __('admin.sort_order') }}</label>
                                                <input type="number" 
                                                       name="sort_orders[{{ $highlight->id }}]" 
                                                       value="{{ $highlight->pivot->sort_order ?? 0 }}"
                                                       min="0" 
                    ...
                    @else
                        <div class="text-center py-8 text-gray-500">
                            ...
                            <a href="{{ route('admin.highlights.create') }}" class="text-purple-600 hover:text-purple-800 mt-2 inline-block">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            checkboxes.forEach(checkbox => {
                checkbox.addEventListener('change', function() {
                    ...
                    const sortInput = document.querySelector(`input[name="sort_orders[${highlightId}]"]`);
