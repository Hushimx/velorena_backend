...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold" style="color: var(--brand-brown);">{{ __('admin.support_ticket_details') }}
                </h1>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="card">
                    <div class="card-header">
                        <h3 class="text-lg font-semibold">{{ __('admin.ticket_information') }}</h3>
                    </div>
                    <div class="card-body">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="form-label"
                                    style="color: #111827 !important;">{{ __('admin.status') }}</label>
                                <div>
                                    <span style="color: #111827 !important;"
                                        class="badge badge-{{ $supportTicket->status === 'open' ? 'success' : ($supportTicket->status === 'closed' ? 'secondary' : ($supportTicket->status === 'resolved' ? 'info' : 'primary')) }}">
                            ...
                            <div>
                                <label class="form-label"
                                    style="color: #111827 !important;">{{ __('admin.priority') }}</label>
                                <div>
                                    <span style="color: #111827 !important;"
                                        class="badge badge-{{ $supportTicket->priority === 'urgent' ? 'danger' : ($supportTicket->priority === 'high' ? 'warning' : ($supportTicket->priority === 'medium' ? 'info' : 'secondary')) }}">
                            ...
                            <div>
                                <label class="form-label"
                                    style="color: #111827 !important;">{{ __('admin.category') }}</label>
                                <p class="text-gray-900">{{ ucfirst(str_replace('_', ' ', $supportTicket->category)) }}</p>
                            </div>
                            <div>
                                <label class="form-label"
                                    style="color: #111827 !important;">{{ __('admin.created_at') }}</label>
                                <p class="text-gray-900">{{ $supportTicket->created_at->format('Y-m-d H:i') }}</p>
                            </div>
                        </div>

                        <div class="mt-4">
                            <label class="form-label" style="color: #111827 !important;">{{ __('admin.subject') }}</label>
                            <p class="text-lg font-semibold text-gray-900">{{ $supportTicket->subject }}</p>
                        </div>

                        <div class="mt-4">
                            <label class="form-label"
                                style="color: #111827 !important;">{{ __('admin.description') }}</label>
                        ...
                        @if ($supportTicket->attachments && count($supportTicket->attachments) > 0)
                            <div class="mt-4">
                                <label class="form-label"
                                    style="color: #111827 !important;">{{ __('admin.attachments') }}</label>
                ...
                <div class="card">
                    <div class="card-header">
                        <h3 class="text-lg font-semibold">{{ __('admin.conversation') }}</h3>
                    </div>
                    <div class="card-body">
                        @forelse($supportTicket->replies as $reply)
                            <div
                                class="border-l-4 {{ $reply->author_type === 'admin' ? 'border-blue-500' : 'border-gray-300' }} pl-4 mb-6">
                                <div class="flex justify-between items-start mb-2">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        @endif
                                        <span
                                            class="badge badge-{{ $reply->author_type === 'admin' ? 'primary' : 'secondary' }}">
                                            {{ $reply->author_type === 'admin' ? __('admin.admin') : __('admin.customer') }}
                                        </span>
                                    </div>
                                    <span
                                        class="text-sm text-gray-500">{{ $reply->created_at->format('M d, Y H:i') }}</span>
                                </div>
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="card">
                    <div class="card-header">
                        <h3 class="text-lg font-semibold">{{ __('admin.customer_information') }}</h3>
                    </div>
                    <div class="card-body">
                        <div class="space-y-4">
                            <div>
                                <label class="form-label"
                                    style="color: #111827 !important;">{{ __('admin.customer_name') }}</label>
                                <p class="text-gray-900">
                                    {{ $supportTicket->user->full_name ?? ($supportTicket->user->company_name ?? $supportTicket->user->email) }}
                                </p>
                            </div>
                            <div>
                                <label class="form-label"
                                    style="color: #111827 !important;">{{ __('admin.email') }}</label>
                                <p class="text-gray-900">{{ $supportTicket->user->email }}</p>
                            </div>
                            @if ($supportTicket->user->phone)
                                <div>
                                    <label class="form-label"
                                        style="color: #111827 !important;">{{ __('admin.phone') }}</label>
                                    <p class="text-gray-900">{{ $supportTicket->user->phone }}</p>
                                </div>
                            @endif
                            <div>
                                <label class="form-label"
                                    style="color: #111827 !important;">{{ __('admin.registration_date') }}</label>
                ...
                <div class="card">
                    <div class="card-header">
                        <h3 class="text-lg font-semibold">{{ __('admin.assignment') }}</h3>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label for="assigned_to" class="form-label"
                                        style="color: #111827 !important;">{{ __('admin.assign_to') }}</label>
                                    <select name="assigned_to" id="assigned_to" class="form-control">
                                        <option value="">{{ __('admin.unassigned') }}</option>
                                        @foreach ($admins as $admin)
                                            <option value="{{ $admin->id }}"
                                                {{ $supportTicket->assigned_to == $admin->id ? 'selected' : '' }}>
                                                {{ $admin->name }}
                                            </option>
                                        @endforeach
                                    </select>
                ...
                <div class="card">
                    <div class="card-header">
                        <h3 class="text-lg font-semibold">{{ __('admin.add_reply') }}</h3>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label for="message" class="form-label"
                                        style="color: #111827 !important;">{{ __('admin.message') }}</label>
                                ...
                                <div class="form-check">
                                    ...
                                    <label class="form-check-label"
                                        for="is_internal">{{ __('admin.internal_note') }}</label>
                                </div>
                ...
                @if ($supportTicket->admin_notes)
                    <div class="card">
                        <div class="card-header">
                            <h3 class="text-lg font-semibold">{{ __('admin.admin_notes') }}</h3>
                        </div>
                        <div class="card-body">
