...
@section('content')
    ...
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">{{ __('admin.support_tickets_management') }}</h1>
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('admin.support-tickets.statistics') }}" class="btn btn-secondary">
                    ...
                    <div class="flex-1 min-w-48">
                        <select class="form-control" onchange="filterByCategory(this.value)">
                            ...
                            <option value="technical" {{ request('category') === 'technical' ? 'selected' : '' }}>
                                {{ __('admin.technical') }}</option>
                            <option value="billing" {{ request('category') === 'billing' ? 'selected' : '' }}>
                                {{ __('admin.billing') }}</option>
                            <option value="general" {{ request('category') === 'general' ? 'selected' : '' }}>
                                {{ __('admin.general') }}</option>
                            <option value="feature_request"
                                {{ request('category') === 'feature_request' ? 'selected' : '' }}>
                                {{ __('admin.feature_request') }}</option>
                            <option value="bug_report" {{ request('category') === 'bug_report' ? 'selected' : '' }}>
                                {{ __('admin.bug_report') }}</option>
                        </select>
                    </div>
                    <div class="flex-1 min-w-48">
                        ...
                        <tbody>
                            @forelse($tickets as $ticket)
                                <tr class="bg-white hover:bg-gray-50">
                                    ...
                                    <td>
                                        <span
                                            class="badge badge-{{ $ticket->priority === 'urgent' ? 'danger' : ($ticket->priority === 'high' ? 'warning' : ($ticket->priority === 'medium' ? 'info' : 'secondary')) }}">
                                            {{ ucfirst($ticket->priority) }}
                                        </span>
                                    </td>
                                    <td>
                                        <span
                                            class="badge badge-{{ $ticket->status === 'open' ? 'success' : ($ticket->status === 'closed' ? 'secondary' : ($ticket->status === 'resolved' ? 'info' : 'primary')) }}">
                                            {{ ucfirst($ticket->status) }}
                                        </span>
                                    </td>
                                    <td>
                                        <span
                                            class="badge badge-{{ $ticket->category === 'technical' ? 'danger' : ($ticket->category === 'billing' ? 'warning' : 'info') }}">
                                    ...
                                    <td>
                                        @if ($ticket->assignedAdmin)
                                            <div>
                                                <div class="font-medium text-gray-900">{{ $ticket->assignedAdmin->name }}
                                                </div>
                                                <div class="text-sm text-gray-500">{{ $ticket->assignedAdmin->email }}
                                                </div>
                                            </div>
                                        @else
                                    ...
                                    <td>
                                        <div>
                                            ...
                                            <div class="text-sm text-gray-500">{{ $ticket->created_at->format('H:i') }}
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                ...
                                <tr class="bg-white">
                                    <td colspan="9" class="text-center py-8">
                                        <div class="flex flex-col items-center">
                                            <i class="fas fa-ticket-alt text-4xl mb-4"
                                                style="color: var(--brand-yellow);"></i>
