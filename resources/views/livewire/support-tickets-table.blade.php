<div>
    ...
    <div class="mb-6 bg-white p-4 rounded-lg shadow-sm">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
                <label for="search"
                    class="block text-sm font-medium text-gray-700 mb-1">{{ trans('support.search') }}</label>
                <input type="text" wire:model.live.debounce.500ms="search" wire:key="search-input"
                    placeholder="{{ trans('support.tickets_search_placeholder') }}"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500">
            </div>
            <div>
                <label for="status_filter"
                    class="block text-sm font-medium text-gray-700 mb-1">{{ trans('support.status_filter') }}</label>
                <select wire:model.live="status_filter" wire:key="status-filter"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500">
                    <option value="">{{ trans('support.all_statuses') }}</option>
                    <option value="open">{{ trans('support.open') }}</option>
                    <option value="in_progress">{{ trans('support.in_progress') }}</option>
                    <option value="pending">{{ trans('support.pending') }}</option>
                    <option value="resolved">{{ trans('support.resolved') }}</option>
                    <option value="closed">{{ trans('support.closed') }}</option>
                </select>
            </div>
            <div>
                <label for="priority_filter"
                    class="block text-sm font-medium text-gray-700 mb-1">{{ trans('support.priority_filter') }}</label>
                <select wire:model.live="priority_filter" wire:key="priority-filter"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500">
                    <option value="">{{ trans('support.all_priorities') }}</option>
                    <option value="low">{{ trans('support.low') }}</option>
                    <option value="medium">{{ trans('support.medium') }}</option>
                    <option value="high">{{ trans('support.high') }}</option>
                    <option value="urgent">{{ trans('support.urgent') }}</option>
                </select>
            </div>
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">
                            {{ trans('support.ticket_number') }}
                        </th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                            {{ trans('support.customer') }}
                        </th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                            {{ trans('support.subject') }}
                        </th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                            {{ trans('support.priority') }}
                        </th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                            {{ trans('support.status') }}
                        </th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                            {{ trans('support.assigned_to') }}
                        </th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                            {{ trans('support.created_at') }}
                        </th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                            {{ trans('actions') }}
                        </th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($tickets as $ticket)
                        <tr class="hover:bg-gray-50" wire:key="ticket-row-{{ $ticket->id }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                ...
                                @else
                                    <span class="text-sm text-gray-500 italic">{{ trans('support.unassigned') }}</span>
                                @endif
                            </td>
                                ...
                                <div class="flex items-center justify-center space-x-4">
                                    ...
                                    @if ($ticket->isOpen())
                                        <button wire:click="confirmDelete({{ $ticket->id }})"
                                            ...
                                            <span>{{ trans('support.delete') }}</span>
                                        </button>
                                    @endif
                                </div>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-4 text-center text-gray-500">
                                {{ trans('support.no_tickets_exist') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
                <div class="mt-3 text-center">
                    ...
                    <h3 class="text-lg leading-6 font-medium text-gray-900 mt-4">
                        {{ trans('support.confirm_delete_title') }}</h3>
                    <div class="mt-2 px-7 py-3">
                        <p class="text-sm text-gray-500">
                            {{ trans('support.confirm_delete_ticket') }}
                        </p>
                    </div>
                    <div class="items-center px-4 py-3">
                        <button wire:click="deleteTicket" wire:loading.attr="disabled"
                            ...
                            <span wire:loading.remove wire:target="deleteTicket">{{ trans('support.delete') }}</span>
                            <span wire:loading wire:target="deleteTicket" class="inline-flex items-center">
