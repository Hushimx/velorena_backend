<div>
    ...
    <div class="mb-6 bg-white p-4 rounded-lg shadow-sm">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                <label for="search"
                    class="block text-sm font-medium text-gray-700 mb-1">{{ trans('designers.search') }}</label>
                <input type="text" wire:model.live.debounce.500ms="search" wire:key="search-input"
                    placeholder="{{ trans('designers.designers_search_placeholder') }}"
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th
                            ...
                            {{ trans('designers.full_name') }}
                        </th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                            {{ trans('designers.email') }}
                        </th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($designers as $designer)
                        <tr class="hover:bg-gray-50" wire:key="designer-row-{{ $designer->id }}">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div
                                        class="h-10 w-10 rounded-lg bg-gray-200 mx-3 flex items-center justify-center ml-3">
                                        <i class="fas fa-user text-gray-400"></i>
                                    </div>
                                    <div>
                                        <div class="text-sm font-medium text-gray-900">{{ $designer->name }}</div>
                                        <div class="text-sm text-gray-500">
                                            {{ Str::limit($designer->bio, 30) ?: trans('designers.not_provided') }}
                                        </div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-900">
                                {{ $designer->phone ?: trans('designers.not_provided') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500">
                                {{ $designer->created_at->format('Y-m-d') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center justify-center space-x-4">
                                    ...
                                    <!-- Delete Button -->
                                    <button wire:click="confirmDelete({{ $designer->id }})"
                                        wire:key="delete-{{ $designer->id }}"
                                        ...
                                        <span>{{ trans('designers.delete') }}</span>
                                    </button>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-4 text-center text-gray-500">
                                {{ trans('designers.no_designers_exist') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
                <div class="mt-3 text-center">
                    ...
                    <h3 class="text-lg leading-6 font-medium text-gray-900 mt-4">
                        {{ trans('designers.confirm_delete_title') }}</h3>
                    <div class="mt-2 px-7 py-3">
                        <p class="text-sm text-gray-500">
                            {{ trans('designers.confirm_delete_designer') }}
                        </p>
                    </div>
                    <div class="items-center px-4 py-3">
                        <button wire:click="deleteDesigner" wire:loading.attr="disabled"
                            ...
                            <span wire:loading.remove
                                wire:target="deleteDesigner">{{ trans('designers.delete') }}</span>
                            <span wire:loading wire:target="deleteDesigner" class="inline-flex items-center">
